...
<c:choose>
    ...
    <c:otherwise>
        ...
        <c:forEach var="comment" items="${commentVOList}" varStatus="status">
            <div id="${cs}_${comment.idx}" style="border: 1px solid; margin-left: ${comment.level * 30}px; width: 50%;">
                <table>
                    <tr>
                        <td>작성자</td>
                        ...
                        <td colspan="2">
                            <c:if test="${isLogin}">
                                <c:if test="${comment.level < 1}">
                                    <button id="${cs}_${comment.idx}_reply_btn" onclick="toggleReply('${cs}', '${comment.idx}', '${param.boardIdx}')" >답글</button>
                                </c:if>
                                <c:if test="${memberId == comment.memberEmail}">
                                    <button id="${cs}_${comment.idx}_update_btn" onclick="updateComment('${cs}', '${comment.idx}')" >수정</button>
                                    <button id="${cs}_${comment.idx}_delete_btn" onclick="deleteComment('${cs}', '${comment.idx}', '${param.boardIdx}')" >삭제</button>
                                </c:if>
                            </c:if>
                        </td>
                    </tr>
                </table>
                <div id="${cs}_${comment.idx}_reply" style="display: none;"></div>
            </div>
            <br/>
        </c:forEach>
    </c:otherwise>
